...
public class Abillity : MonoBehaviour
{
    ...
    protected virtual GameObject FindTarget()  // 공격 대상 탐색
    {
    ...
    protected float DistanceToTarget()    // 플레이어와 타겟 사이 거리 계산
    {
        return Vector2.Distance(player.transform.position, target.transform.position);
    }
 

    protected virtual float AttackDelay()   // 공격 딜레이 계산 (공격속도 기반), 공격속도가 높을수록 딜레이는 짧아짐
    {
    ...
    protected virtual void TryAttack()  // 공격을 시도하는 함수
    {
    ...
    protected virtual void PerformAttack()      // 실제 공격 로직 수행 (이 부분을 상속해서 각 능력마다 다르게 구현 가능)
    {
